...
Открываете `lib/domains.sh`, добавляете домен в оба массива:

```bash
AI_DOMAINS=(
    ...
    "newservice.com"
    "*.newservice.com"     # все субдомены
)

AI_APEX_DOMAINS=(
    ...
    "newservice.com"       # для Blocky (субдомены резолвятся автоматически)
)
```

Запускаете установщик повторно (конфиги перегенерируются):
