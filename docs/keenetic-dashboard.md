...
- если default route на роутере уходит в VPN-интерфейс, эти DNS могут случайно уйти в туннель
...
- DNS провайдера идет через WAN/ISP, а не через активный VPN-туннель
...
### 1. На роутере активен VPN-туннель и он владеет default route

Самая неприятная ловушка на Keenetic: DNS провайдера может начать ходить через VPN-туннель, если тот перехватывает default route.
...
Особенно если на роутере активны несколько VPN-туннелей и сложная маршрутизация.
